...
const AutocompleteField = require('@mapstore/components/data/query/AutocompleteFieldHOC');
const SwitchPanel = require('@mapstore/components/misc/switch/SwitchPanel');
const StringSelector = require('@mapstore/components/misc/StringSelector');
...
class GroupField extends React.Component {
    ...
    renderFilterField = (filterField) => {
        ...
        const deleteButton = filterField.exception ?
            (<OverlayTrigger placement="bottom" overlay={(<Tooltip id={filterField.rowId + "tooltip"}><strong><I18N.Message msgId={filterField.exception || ""}/></strong></Tooltip>)}>
                <Button id="remove-filter-field" className="no-border square-button-md" style={{backgroundColor: "red"}} onClick={() => this.props.actions.onRemoveFilterField(filterField.rowId)}>
            ...
            (<OverlayTrigger placement="top" overlay={(<Tooltip id={filterField.rowId + "tooltip"}><strong>
                <I18N.Message msgId="queryform.attributefilter.delete" /></strong></Tooltip>)}><Button id="remove-filter-field" className="no-border square-button-md" onClick={() => this.props.actions.onRemoveFilterField(filterField.rowId)}>
        ...
        const filterComponent = (
            <FilterField
                ...
                noBootstrap={this.props.noBootstrap}>
                <ComboField
                    attType="list"
                    valueField={'id'}
                    textField={'name'}
        ...
        if (this.props.noBootstrap) {
            return <div key={filterField.rowId}>{filterComponent}</div>;
        }
        return (
    ...
    renderGroupHeader = (groupField) => {
        ...
        if (this.props.noBootstrap) {
            return (
                <div
                    key="filter-logic-header"
                    className="filter-logic-header-text">
                    <div>
                        <I18N.Message msgId={"queryform.attributefilter.group_label_a"}/>
                        &nbsp;<StringSelector
                        ...
                        />&nbsp;
                        <I18N.Message msgId={"queryform.attributefilter.group_label_b"}/>
                    </div>
                    <div>{this.renderGroupButtons(groupField)}</div>
                </div>
            );
        }
        return (
