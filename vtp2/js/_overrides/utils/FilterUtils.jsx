 ...
 */
const {processOGCGeometry, pointElement, polygonElement, lineStringElement, closePolygon } = require("@mapstore/utils/ogc/GML");
const {wfsToGmlVersion} = require('@mapstore/utils/ogc/WFS/base');
const {ogcComparisonOperators, ogcLogicalOperators, ogcSpatialOperators} = require("@mapstore/utils/ogc/Filter/operators");
const { read } = require('@mapstore/utils/ogc/Filter/CQL/parser');
    ...
    */
    ...
    processCQLSpatialFilter: function(objFilter) {
        if (objFilter.spatialField.operation === 'BBOX') {
            const extent = objFilter.spatialField && objFilter.spatialField.geometry && objFilter.spatialField.geometry.extent;
            if (!extent) {
                return '';
            }
            return 'BBOX(' + objFilter.spatialField.attribute + ',' + extent.join(',') + ')';
        }
